...
void resetOnDeath(std::vector<Spike>& spikes, std::vector<Coin>& coins, Door& door) {
...
int main() {
    ...
    if (jump_sound_loaded) { jump_sound.setVolume(50.f); jump_sound.setPitch(1.4f); }
    ...
    if (win_sound_loaded) { win_sound.setVolume(50.f); win_sound.setPitch(1.2f); }

    Player player;
    std::vector<Platform> platforms;
    std::vector<Spike>    spikes;
    std::vector<Coin>     coins;
    std::vector<Wall>     walls;   // CHANGED
    Door door(0, 0);
    ...
    while (window.isOpen())
    {
        ...
        if (!level_complete) {
            ...
            for (auto& spike : spikes) {
                ...
                if (spike.kills(player)) {
                    player.die();
                    resetOnDeath(spikes, coins, door);
                    break;
                }
            }
        ...
        for (const auto& c : coins) {
            if (!c.fake) { total_real++; if (c.collected) collected++; }
        }

        death_text.setString("Death: " + std::to_string(player.death_count));
        coin_text.setString(total_real > 0 ? "Coins: " + std::to_string(collected) + "/" + std::to_string(total_real) : "");
